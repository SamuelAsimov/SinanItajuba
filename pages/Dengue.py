...
def query_unidades():
    return f'''
SELECT cnes.nome AS Unidade, COUNT(doenca.ID_UNIDADE) AS QTD FROM cnes_itajuba cnes
INNER JOIN {tabela} AS doenca ON doenca.ID_UNIDADE = cnes.id
...
def query_testes():
    return f'''
SELECT be.nome as Escolaridade, bs.Nome as Sexo, br.Nome as Raça FROM {tabela} doenca
...
df_unidades = conn.query(query_unidades(), ttl=600)
...
# Agora podemos usar o método dt.strftime() para formatar a data
...
fig_unidade = criar_grafico_horizontal(df_unidades, 'QTD', 'Unidade', "Casos por unidade", True, 400)
col11, col22 = st.columns([1,1])

with col11:
    st.altair_chart(fig_unidade, use_container_width=True)
with col22:
